...
This separately authored research companion develops a theorem package
suggested by a chronological collection of Collatz research notes.  The notes
are intellectual provenance, not theorem authority: every statement retained
here is rederived from the displayed definitions.  The starting sketches
contained a substantial finite core, together with four precise defects:
mutually incompatible composition orders, nonunital transformation
semigroups called monoids, infinite path sums placed in a finite-support
algebra, and a prefix $3$-adic cocycle identified with a forward affine
intercept without reversing the exponent word.  The constructions below keep
the original odd-step coordinates and repair those defects at the point where
they occur.

The standard algebraic vocabulary is crosswalked against the definitions of
free monoids, semirings, formal power series, weighted paths, and
transformation monoids in
...
here.  No claim of priority is made for results independently reconstructed in
this paper.
...
\]
A separate coefficient-label convention assigns $00$ to every even branch
and $11$ to every odd branch.  On an odd-to-odd block path it instead gives
\[
 d_{\mathrm{coeff}}(p)
...
comparison with Tao's displayed definition gives the final identity.  No
external reversal assertion is used: the displayed reindexing is the complete
comparison map.
\end{proof}
...
\subsection{Gaussian prefix angles and the Collatz--Machin relation lattice}
\label{subsec:collatz-machin-relations}

The individual summands of the prefix cocycle carry a second, multiplicative
structure.  It is independent of whether the exponent word is realized by a
particular positive source; Proposition~\ref{prop:weighted-word-source-fibres}
then shows that every word considered below has infinitely many such sources.

\begin{definition}[Collatz--Machin relation lattice]
\label{def:collatz-machin-relation-lattice}
Let $w=(a_1,\ldots,a_n)$ be a nonempty positive exponent word.  For
$1\leq k\leq n$, define
\begin{equation}
\label{eq:collatz-machin-angle-data}
 r_k(w)=\frac{3^{k-1}}{2^{A_k(w)}},\qquad
 \vartheta_k(w)=\arctan r_k(w),\qquad
 z_k(w)=2^{A_k(w)}+i3^{k-1},
\end{equation}
where $\arctan$ is the principal value in $(0,\pi/2)$, and put
\begin{equation}
\label{eq:collatz-machin-unit-product}
 u_k(w)=\frac{z_k(w)}{\overline{z_k(w)}}
       =\frac{1+i r_k(w)}{1-i r_k(w)}
       =e^{2i\vartheta_k(w)}.
\end{equation}
The integer relation lattice is
\begin{equation}
\label{eq:collatz-machin-relation-lattice}
 \operatorname{Rel}_{\pi}(w)=
 \left\{m\in\mathbb Z^n:
 \sum_{k=1}^n m_k\vartheta_k(w)\in\pi\mathbb Q\right\}.
\end{equation}
\end{definition}

Write
\[
 \mathcal U_1(\mathbb Q(i))
 =\{x\in\mathbb Q(i)^\times:x\overline{x}=1\},
 \qquad \mu_4=\{1,-1,i,-i\}.
\]
Exponentiating \eqref{eq:collatz-machin-relation-lattice} gives the exact
criterion
\begin{equation}
\label{eq:collatz-machin-torsion-criterion}
 m\in\operatorname{Rel}_{\pi}(w)
 \quad\Longleftrightarrow\quad
 \prod_{k=1}^n u_k(w)^{m_k}\in\mu_4.
\end{equation}
Indeed the product lies in $\mathbb Q(i)^\times$, whose roots of unity are
exactly $\mu_4$; conversely a value in $\mu_4$ places the angle sum in
$(\pi/4)\mathbb Z$.  This is a multiplicative torsion test, not an additive
unit equation.

The fixed-instance theory is classical.  Conway, Radin, and Sadun reconstruct
the $d=1$ St\o rmer theory by orienting the Gaussian primes above
$p\equiv1\pmod4$ and prove the resulting prime angles independent modulo
$\pi$ \cite[Theorems~1--2 and Section~2]{ConwayRadinSadun1999}.  Calcut's
Main Lemma and Corollaries~1--3 give the same $\mu_4$ restriction and exclude
a rational multiple of $\pi$ for a single rational tangent other than
$0,\pm1$ \cite{Calcut2009}.  We now type that factorization as an exact
morphism for the present prefix family.

\begin{theorem}[Gaussian coordinates for every fixed word]
\label{thm:collatz-machin-fixed-word-kernel}
For each rational prime $p\equiv1\pmod4$, choose one Gaussian prime
$\varpi_p$ above $p$, so $p=\varpi_p\overline{\varpi_p}$.  There is an
isomorphism
\begin{equation}
\label{eq:collatz-machin-gaussian-valuation-isomorphism}
 \nu_{\mathrm G}:\mathcal U_1(\mathbb Q(i))/\mu_4
 \xrightarrow{\ \sim\ }
 \bigoplus_{p\equiv1\ (4)}\mathbb Z,
 \qquad
 [x]\longmapsto\bigl(v_{\varpi_p}(x)\bigr)_p,
\end{equation}
with inverse
\[
 (e_p)_p\longmapsto
 \left[\prod_p
 \left(\frac{\varpi_p}{\overline{\varpi_p}}\right)^{e_p}\right].
\]
For the word $w$, define
\begin{equation}
\label{eq:collatz-machin-signed-valuation-matrix}
 E_w:\mathbb Z^n\longrightarrow
 \bigoplus_{p\equiv1\ (4)}\mathbb Z,
 \qquad
 (E_w)_{p,k}
 =v_{\varpi_p}(z_k(w))-v_{\overline{\varpi_p}}(z_k(w)).
\end{equation}
Then
\begin{equation}
\label{eq:collatz-machin-exact-kernel}
 \operatorname{Rel}_{\pi}(w)=\ker E_w.
\end{equation}
Every fibre over $y\in\operatorname{im}E_w$ is the coset
$m+\operatorname{Rel}_{\pi}(w)$ for any $m$ with $E_w(m)=y$, and the induced
map
\[
 \mathbb Z^n/\operatorname{Rel}_{\pi}(w)
 \xrightarrow{\ \sim\ }\operatorname{im}E_w
\]
has inverse $y\mapsto m+\operatorname{Rel}_{\pi}(w)$.  Changing the chosen
orientation over one split prime changes only the sign of that coordinate.
Every fibre over a point outside $\operatorname{im}E_w$ is empty.
\end{theorem}

\begin{proof}
Unique factorization in $\mathbb Z[i]$ gives a finite prime factorization for
every $x\in\mathbb Q(i)^\times$.  If $x\overline{x}=1$, the valuations at the
two primes above a split $p$ are opposite.  A prime $p\equiv3\pmod4$ is fixed
by conjugation, and $1+i$ is associated to its conjugate, so their valuations
must be zero.  The remaining factor is a Gaussian unit.  Quotienting by
$\mu_4$ therefore leaves exactly one integer exponent for each oriented split
prime.  This proves that \eqref{eq:collatz-machin-gaussian-valuation-isomorphism}
is well defined, injective, and surjective, and the displayed product proves
its inverse formula.  All coordinate vectors have finite support.

Define
\[
 \overline{\Gamma}_w:\mathbb Z^n\longrightarrow
 \mathcal U_1(\mathbb Q(i))/\mu_4,
 \qquad
 m\longmapsto\left[\prod_{k=1}^n u_k(w)^{m_k}\right].
\]
It is a homomorphism, and
\eqref{eq:collatz-machin-torsion-criterion} says that its kernel is
$\operatorname{Rel}_{\pi}(w)$.  Since
\[
 v_{\varpi_p}(u_k(w))
 =v_{\varpi_p}(z_k(w))-v_{\varpi_p}(\overline{z_k(w)})
 =v_{\varpi_p}(z_k(w))-v_{\overline{\varpi_p}}(z_k(w)),
\]
the composite $\nu_{\mathrm G}\circ\overline{\Gamma}_w$ is exactly $E_w$.
The kernel formula follows because $\nu_{\mathrm G}$ is injective.  The fibre
and inverse-on-image statements are the first isomorphism theorem written in
coordinates; they introduce no choice after passage to the quotient coset.
\end{proof}

The theorem makes every fixed word decidable after exact factorization of the
finitely many norms
\[
 N(z_k(w))=2^{2A_k(w)}+3^{2(k-1)}.
\]
Its content is not the conjecture below: the latter asks for a uniform rank
classification as both the word and these factorizations vary.

\begin{theorem}[unweighted Collatz--Machin non-torsion]
\label{thm:collatz-machin-unweighted-nontorsion}
For every nonempty positive exponent word $w=(a_1,\ldots,a_n)$,
\[
 \sum_{k=1}^n\vartheta_k(w)\notin\pi\mathbb Q.
\]
Equivalently, $(1,\ldots,1)\notin\operatorname{Rel}_{\pi}(w)$.
\end{theorem}

\begin{proof}
Put
\[
 Z(w)=\prod_{k=1}^n z_k(w)=X+iY\in\mathbb Z[i].
\]
Modulo three, every factor with $k\geq2$ is the nonzero real number
$2^{A_k(w)}$, while the first factor is $2^{A_1(w)}+i$.  Hence both $X$ and
$Y$ are nonzero modulo three.  It follows that $Z/\overline Z$ is neither
$1$ nor $-1$, since those two equalities would force respectively $Y=0$ or
$X=0$.

Modulo two, every factor is $i$, because $A_k(w)\geq1$ and $3^{k-1}$ is odd.
Thus $Z\equiv i^n\pmod2$: $X$ and $Y$ have opposite parity.  The equalities
$Z/\overline Z=i$ and $Z/\overline Z=-i$ would force $X=Y$ and $X=-Y$,
respectively, both impossible for integers of opposite parity.  Therefore
$Z/\overline Z\notin\mu_4$.  But
\[
 \frac{Z}{\overline Z}=\prod_{k=1}^n u_k(w),
\]
so \eqref{eq:collatz-machin-torsion-criterion} completes the proof.
\end{proof}

The coefficients cannot be removed from the statement: the legal word
$(1,1)$ has a genuine weighted relation.

\begin{theorem}[the exact exceptional two-letter lattice]
\label{thm:collatz-machin-exceptional-seed}
For $w=(1,1)$,
\begin{equation}
\label{eq:collatz-machin-seed}
 \operatorname{Rel}_{\pi}(1,1)=\mathbb Z(2,1),
 \qquad
...
Here
\[
 z_1=2+i,\qquad z_2=4+3i=i(2-i)^2=i\overline{z_1}^{\,2},
\]
and therefore
\[
 (2+i)^2(4+3i)=25i,\qquad u_2=-u_1^{-2}.
\]
It follows that $u_1^2u_2=-1$ and hence $(2,1)$ belongs to the relation
lattice.  Since both angles lie in $(0,\pi/4)$, the resulting congruence is
the displayed equality with $\pi/2$, not merely equality modulo $\pi$.

The Gaussian prime $2+i$ divides the numerator but not the denominator of
$u_1=(2+i)/(2-i)$, so $v_{2+i}(u_1^d)=d$.  Every element of $\mu_4$ has
$v_{2+i}=0$; hence $u_1^d\in\mu_4$ forces $d=0$.  For arbitrary $(m_1,m_2)$,
\[
 u_1^{m_1}u_2^{m_2}=(-1)^{m_2}u_1^{m_1-2m_2}
\]
lies in $\mu_4$ exactly when $m_1-2m_2=0$.  Hence the full lattice, including
all zero and nonzero right-hand sides, is $\mathbb Z(2,1)$.
\end{proof}

The same identity occurs in Gasull, Luca, and Varona as
\[
 \arctan(1/2)+\tfrac12\arctan(3/4)=\pi/4
\]
inside their classification of two-term, nonzero-right-hand-side formulas
with $2$-integer arguments \cite[Theorem~1]{GasullLucaVarona2023}.  Their
theorem does not state the full higher-length prefix classification.

\begin{theorem}[exact full-grid relation lattice]
\label{thm:collatz-machin-finite-census}
Let
\[
 \mathcal G_{15,60}
 =\{(k,A):1\leq k\leq15,\ k\leq A\leq60\},
 \qquad |\mathcal G_{15,60}|=795,
\]
and attach
 $z_{k,A}=2^A+i3^{k-1}$ and
 $\vartheta_{k,A}=\arctan(3^{k-1}/2^A)$ to each column.  Define
\[
 L_{15,60}:=\operatorname{Rel}_{\pi}(\mathcal G_{15,60})
 =\left\{m\in\mathbb Z^{\mathcal G_{15,60}}:
 \sum_{(k,A)\in\mathcal G_{15,60}}m_{k,A}\vartheta_{k,A}
 \in\pi\mathbb Q\right\}.
\]
The complete integer relation lattice on these 795 angles is
\begin{equation}
\label{eq:collatz-machin-grid-lattice}
 L_{15,60}=\mathbb Z\mathbf r_1\oplus\mathbb Z\mathbf r_2,
\end{equation}
where
\begin{align*}
 \mathbf r_1&=2e_{(1,1)}+e_{(2,2)},\\
 \mathbf r_2&=2e_{(1,1)}+e_{(1,2)}+e_{(1,3)}+e_{(3,5)}.
\end{align*}
The two exact Gaussian certificates are
\[
 (2+i)^2(4+3i)=25i
\]
and
\[
 (2+i)^2(4+i)(8+i)(32+9i)=5525i.
\]
Consequently every positive word with $n\leq15$ and $A_n(w)\leq60$ has
exactly the lattice predicted in Conjecture~\ref{conj:collatz-machin-rigidity};
there is no bound on the coefficients of a tested relation.
\end{theorem}

\begin{proof}
The portable certificate
\path{certificates/arctangent_relation_lattice_checks.py} uses only exact
integer and finite-field arithmetic.  It takes all 1,008 primes
$q\leq38{,}821$ with $q\equiv1\pmod{12}$.  A prime is retained when every
$z_{k,A}/\overline{z_{k,A}}$ has good reduction in $\mathbb F_q$.  For each
of the 794 retained primes, choose a square root of $-1$ and apply the cubic
character
\[
 \chi_q(x)=x^{(q-1)/3}.
\]
Because $4$ divides $(q-1)/3$, this character kills $\mu_4$.  After identifying
its values with $\mathbb F_3$, every genuine relation therefore reduces to a
vector in the kernel of the resulting $794\times795$ matrix over
$\mathbb F_3$.  Exact row reduction gives rank 793 and nullity two.  The rank
before the last prime is 792; $38{,}821$ is the first prime giving full rank.
The reductions of $\mathbf r_1$ and $\mathbf r_2$ are independent kernel
vectors, so they are the complete mod-three kernel.

Let
\[
 \overline\Gamma_{\mathcal G}:\mathbb Z^{\mathcal G_{15,60}}
 \longrightarrow\mathcal U_1(\mathbb Q(i))/\mu_4
\]
be the product map defined as in
Theorem~\ref{thm:collatz-machin-fixed-word-kernel}.  Let $L$ be its kernel and
$\mathcal L_0=\mathbb Z\mathbf r_1+\mathbb Z\mathbf r_2$.  The displayed
Gaussian products prove $\mathcal L_0\subseteq L$.  The character calculation
gives, for every $\ell\in L$, an expression $\ell=r+3z$ with
$r\in\mathcal L_0$ and
$z\in\mathbb Z^{\mathcal G_{15,60}}$.  In the target
$\mathcal U_1(\mathbb Q(i))/\mu_4$ this implies
$\overline\Gamma_{\mathcal G}(z)^3=1$.
That target is torsion-free: if $[x]^d=1$, then $x^d\in\mu_4$, hence
$x^{4d}=1$ and $x\in\mu_4$.  Therefore $z\in L$ and
$L=\mathcal L_0+3L$.

On the two coordinates $(2,2)$ and $(1,2)$, the columns of
$(\mathbf r_1,\mathbf r_2)$ form the identity matrix.  Thus $\mathcal L_0$ is a direct
summand of the ambient integer lattice.  The subgroup $L/\mathcal L_0$ is
consequently finitely generated and free, while
$L/\mathcal L_0=3(L/\mathcal L_0)$; reducing a free basis modulo three forces
$L/\mathcal L_0=0$.
This proves \eqref{eq:collatz-machin-grid-lattice} over the integers.

A prefix chain contains at most one column for each $k$.  In
$a\mathbf r_1+b\mathbf r_2$, the exact coefficients at both $(1,2)$ and
$(1,3)$ equal $b$.
Thus $b\ne0$ is incompatible with support on a prefix chain.  A nonzero
multiple of $\mathbf r_1$ is supported
on a chain exactly when that chain contains $(1,1)$ and $(2,2)$, equivalently
when $(a_1,a_2)=(1,1)$.  The coordinate embedding
$e_k\mapsto e_{(k,A_k(w))}$ identifies the word relation lattice with the
intersection of $L_{15,60}$ and its chain coordinate subspace.  This proves
the final assertion.
\end{proof}

The certificate pins the grid, prime lists, full trit matrix, independent-row
list, and pivot columns by SHA-256.  It also checks all 631,230 character
entries, both Gaussian products, the unit saturation minor, and all nine
mod-three combinations of the script vectors named \texttt{R1}, \texttt{R2},
which are $\mathbf r_1,\mathbf r_2$ above.  It certifies no point outside the
displayed grid.

\begin{conjecture}[Collatz--Machin rigidity]
\label{conj:collatz-machin-rigidity}
For every nonempty positive exponent word $w=(a_1,\ldots,a_n)$,
\[
 \operatorname{Rel}_{\pi}(w)=
 \begin{cases}
 \mathbb Z(2,1,0,\ldots,0),
   & n\geq2\text{ and }(a_1,a_2)=(1,1),\\
 \{0\},&\text{otherwise}.
 \end{cases}
\]
\end{conjecture}

The fixed-word theorem proves that this is exactly a uniform signed-valuation
rank statement for the Gaussian integers
$2^{A_k(w)}+i3^{k-1}$.  Theorems
\ref{thm:collatz-machin-unweighted-nontorsion} and
\ref{thm:collatz-machin-exceptional-seed} prove two all-length boundaries, and
Theorem~\ref{thm:collatz-machin-finite-census} proves the complete result for
$n\leq15$, $A_n\leq60$.  Conway--Radin--Sadun, Section~2, supplies the
classical Gaussian-UFD coordinates for fixed rational angles; Gasull--Luca--
Varona, Theorem~1, supplies its stated two-term classification and the
$\mathbf r_1$ identity.  Neither cited statement establishes the 795-column
lattice or the uniform prefix-word classification.  This is a source-scope
comparison, not a novelty or priority claim.  The conjecture concerns
relations among prefix angles; it asserts neither Collatz termination nor any
endpoint property of an orbit.
...
\subsection{Typed finite actions and the two source obstructions}
...
\subsection{Exact boundary of the construction}

Several proposed extensions fail before any Collatz-specific issue arises.
The exact failures determine which algebraic structures have actually been
built.
...
\item an exact Gaussian signed-valuation map for every prefix-angle relation
lattice, an all-word unweighted non-torsion theorem, the exceptional
two-letter lattice, and a coefficient-unbounded full-grid calculation;
...
\begin{nonclaim}
The displayed data do not construct a path groupoid of local homeomorphisms,
a groupoid $C^*$-algebra, a Leavitt or Cuntz--Krieger algebra, a quotient
category $\operatorname{QGr}$, a Hopf or bialgebra structure, a crossed
coefficient product, or an operator-algebra completion.  Names and analogies
do not supply the absent objects or maps.  None of the theorems in this paper
proves termination of every positive Collatz orbit.
\end{nonclaim}

The portable certificate
\path{certificates/finite_actions_checks.py} verifies bounded instances of the
block inverse, grading fibres, cocycle and reversal formulas, the
noninjective undecorated $\rho$ example, source congruences, predecessor
...
and the explicit fixed-point-loop kernel relation.  It has no private-corpus
dependency.  Source-identity pins belong to the nonportable critical
workspace and are deliberately not copied into this package.  The finite
certificate does not independently prove the infinite, analytic,
faithfulness, or exact-kernel theorems printed above.

The separate portable certificate
\path{certificates/arctangent_relation_lattice_checks.py} proves the complete
integer relation lattice on the 795-column grid
$1\leq k\leq15$, $k\leq A\leq60$ by exact cubic-character rows, finite-field
rank, a saturated-lattice lift, and two Gaussian product identities.  It has
no coefficient search bound.  It does not certify any column outside that
grid or the all-length conjecture.
